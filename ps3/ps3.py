...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    hand_dealt=deal_hand(6)
    hand_length = calculate_handlen(hand_dealt)
    display_hand(hand_dealt)
    word_entered=input('Please enter the word : ',word_entered)

    if word_entered=='!!':
        return True
    else:
        if is_valid_word(word_entered,hand_dealt,word_list):
            get_word_score(word_entered,hand_length)
            update_hand(hand_dealt,word_entered)
        else:
